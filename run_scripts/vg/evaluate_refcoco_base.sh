...
data=$DATADIR/refcoco_data/refcoco_val.tsv

path=$1

result_path=../../results/refcoco-joint-nodet-base
description=tep
...
data=$DATADIR/refcoco_data/refcoco_testA.tsv
...
data=$DATADIR/refcoco_data/refcoco_testB.tsv
...
python3 -m torch.distributed.launch --master_port=${MASTER_PORT} ../../evaluate.py \
    ...
    --fp16 \
    --num-workers=0 \
    --model-overrides="{\"description\":\"${description}\",\"data\":\"${data}\",\"bpe_dir\":\"${bpe_dir}\",\"selected_cols\":\"${selected_cols}\"}" | tee -a ${result_path}/log_B.txt
