...
class OFAModel(TransformerModel):
    ...
    def forward(
        ...
        task_idf = None
        if task_name is not None and hasattr(self, task_name):
            task_idf = getattr(self, task_name)
            for task in self.tasks:
                task_idf = task_idf +  getattr(self, task ) * 0
        ...
        self.enc_timer[1] += 1
        self.enc_timer[0] += start.elapsed_time(end)
        ...
        torch.cuda.synchronize()
        self.dec_timer[1] += 1
        self.dec_timer[0] += start.elapsed_time(end)
        ...
        torch.cuda.synchronize()
        self.cls_timer[1] += 1
        self.cls_timer[0] += start.elapsed_time(end)

        # print(self.enc_timer[0] / self.enc_timer[1], self.dec_timer[0] / self.dec_timer[1]  ,self.cls_timer[0] / self.cls_timer[1])
