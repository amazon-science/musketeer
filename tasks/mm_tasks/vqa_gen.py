...
class VqaGenTask(OFATask):
    def __init__(self, cfg: VqaGenConfig, src_dict, tgt_dict,  data=None):
        ...
        if getattr(self.cfg, 'vqa_ans2label_file') is not None:
            self.ans2label_dict = pickle.load(open(self.cfg.vqa_ans2label_file, "rb"))

        elif self.cfg.ans2label_file is not None:
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = VqaGenDataset(
            ...
            description=getattr(self.cfg, 'description', 'base')
    ...
    def build_shared_model(self, cfg, model):
        answer_item_list = []
        self.index2ans = {}
        self.constraint_trie = Trie(self.tgt_dict.eos())
        for i, answer in enumerate(self.ans2label_dict.keys()):
